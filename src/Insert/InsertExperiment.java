                    ...
                    if (rs.next()) {
                        ...
                        if (rs.next()) {
                            ...
                            if (rs.next()) {
                                ...
                                statement.executeUpdate("INSERT INTO [dbo].[Experiment1] ([ExperimentType_ID],[location_ID],[Block_ID],[Plot_ID],[Year],[Note]) VALUES(" + ExperimentType_ID + "," + Origin_ID + "," + Block_ID + ",'" + Plot_ID + "',17,'"+Note+"')");
                            } else {
                                statement.executeUpdate("INSERT INTO [dbo].[Experiment1] ([ExperimentType_ID],[location_ID],[Block_ID],[Plot_ID],[Year],[Note]) VALUES(" + ExperimentType_ID + "," + Origin_ID + ",Null,'" + Plot_ID + "',17,'"+Note+"')");
                            }
                            counter++;
                            rs = statement.executeQuery("select IDENT_CURRENT('Experiment1')");
                            if (rs.next()) {
                                Experiment_ID = rs.getInt(1);
                                String q = "select ID from View_Package1 where replace(Name,' ','') = '" + Name + "' and replace(Origin,' ','') = '" + FileOrigin + "'";
